	...
	)
{
	...
	int32	remain;			/* Bytes remaining 		*/
	int32	pos;			/* Current position in file	*/
	int32	readcount;		/* Total bytes read		*/
	...
	readcount = 0;
	while(remain > 0) {
		msg.rf_pos = htonl(pos);
		if(remain > RF_DATALEN) {
			msg.rf_len = htonl(RF_DATALEN);
			remain = remain - RF_DATALEN;
			pos += RF_DATALEN;
		}
		else {
			msg.rf_len = htonl(remain);
			pos += remain;
			remain = 0;
		}
		...
		readcount += htonl(resp.rf_len);

		/* If the response length was less than request */
		/* We have reached EOF				*/
		if(ntohl(resp.rf_len) < ntohl(msg.rf_len)) {
			break;
		}
	}
